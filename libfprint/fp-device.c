...
G_DEFINE_TYPE_EXTENDED(
    FpDevice, fp_device, G_TYPE_OBJECT, G_TYPE_FLAG_ABSTRACT,
...
static gboolean fp_device_cancel_in_idle_cb(gpointer user_data) {
 ...
 * with the task. */
static void fp_device_cancelled_cb(GCancellable *cancellable, FpDevice *self) {
  FpDevicePrivate *priv = fp_device_get_instance_private(self);

  priv->current_idle_cancel_source = g_idle_source_new();
  g_source_set_callback(priv->current_idle_cancel_source,
                        fp_device_cancel_in_idle_cb, self, NULL);
...
static void fp_device_task_cancelled_cb(GCancellable *cancellable,
                                        FpDevice *self) {
  FpDevicePrivate *priv = fp_device_get_instance_private(self);
...
static void setup_task_cancellable(FpDevice *device) {
  ...
  if (cls->cancel) {
    priv->current_cancellable_id =
        g_cancellable_connect(priv->current_cancellable,
                              G_CALLBACK(fp_device_cancelled_cb), device, NULL);
  }
  ...
  if (g_task_get_cancellable(priv->current_task)) {
    priv->current_task_cancellable_id = g_cancellable_connect(
        g_task_get_cancellable(priv->current_task),
        G_CALLBACK(fp_device_task_cancelled_cb), device, NULL);
  }
}

static void fp_device_constructed(GObject *object) {
...
static void fp_device_finalize(GObject *object) {
  FpDevice *self = (FpDevice *)object;
  FpDevicePrivate *priv = fp_device_get_instance_private(self);
...
static void fp_device_get_property(GObject *object, guint prop_id,
...
static void fp_device_set_property(GObject *object, guint prop_id,
...
static void device_idle_probe_cb(FpDevice *self, gpointer user_data) {
...
static void fp_device_async_initable_init_async(GAsyncInitable *initable,
                                                int io_priority,
   ...
   */
  g_source_set_name(
      fpi_device_add_timeout(self, 0, device_idle_probe_cb, NULL, NULL),
      "libusb probe in idle");
}

static gboolean fp_device_async_initable_init_finish(GAsyncInitable *initable,
                                                     GAsyncResult *res,
                                                     GError **error) {
...
static void fp_device_class_init(FpDeviceClass *klass) {
  ...
  properties[PROP_NR_ENROLL_STAGES] = g_param_spec_uint(
      "nr-enroll-stages", "EnrollStages",
      "Number of enroll stages needed on the device", 0, G_MAXUINT, 0,
      G_PARAM_STATIC_STRINGS | G_PARAM_READABLE);

  properties[PROP_SCAN_TYPE] = g_param_spec_enum(
      "scan-type", "ScanType", "The scan type of the device", FP_TYPE_SCAN_TYPE,
      FP_SCAN_TYPE_SWIPE, G_PARAM_STATIC_STRINGS | G_PARAM_READABLE);

  properties[PROP_FINGER_STATUS] = g_param_spec_flags(
      "finger-status", "FingerStatus", "The status of the finger",
      FP_TYPE_FINGER_STATUS_FLAGS, FP_FINGER_STATUS_NONE,
      G_PARAM_STATIC_STRINGS | G_PARAM_READABLE);

  properties[PROP_TEMPERATURE] = g_param_spec_enum(
  ...
  properties[PROP_DRIVER] =
      g_param_spec_string("driver", "Driver", "String describing the driver",
  ...
  properties[PROP_NAME] = g_param_spec_string(
      "name", "Device Name", "Human readable name for the device", NULL,
      G_PARAM_STATIC_STRINGS | G_PARAM_READABLE);

  properties[PROP_OPEN] = g_param_spec_boolean(
      "open", "Opened", "Whether the device is open or not", FALSE,
      G_PARAM_STATIC_STRINGS | G_PARAM_READABLE);

  properties[PROP_REMOVED] = g_param_spec_boolean(
      "removed", "Removed",
  ...
  signals[REMOVED_SIGNAL] =
      g_signal_new("removed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0,
   ...
   * Stability: private
   */
  properties[PROP_FPI_ENVIRON] = g_param_spec_string(
      "fpi-environ", "Virtual Environment",
      "Private: The environment variable for the virtual device", NULL,
   ...
   * Stability: private
   */
  properties[PROP_FPI_USB_DEVICE] = g_param_spec_object(
      "fpi-usb-device", "USB Device", "Private: The USB device for the device",
      G_USB_TYPE_DEVICE,
   ...
   * Stability: private
   */
  properties[PROP_FPI_UDEV_DATA_SPIDEV] = g_param_spec_string(
      "fpi-udev-data-spidev", "Udev data: spidev path",
      "Private: The path to /dev/spidevN.M", NULL,
      G_PARAM_STATIC_STRINGS | G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
  /**
   ...
   * Stability: private
   */
  properties[PROP_FPI_UDEV_DATA_HIDRAW] = g_param_spec_string(
      "fpi-udev-data-hidraw", "Udev data: hidraw path",
      "Private: The path to /dev/hidrawN", NULL,
      G_PARAM_STATIC_STRINGS | G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
   ...
   * Stability: private
   */
  properties[PROP_FPI_DRIVER_DATA] = g_param_spec_uint64(
      "fpi-driver-data", "Driver Data",
      "Private: The driver data from the ID table entry", 0, G_MAXUINT64, 0,
      G_PARAM_STATIC_STRINGS | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY);

  g_object_class_install_properties(object_class, N_PROPS, properties);
}

static void fp_device_init(FpDevice *self) {}
...
static void enroll_data_free(FpEnrollData *data) {
 ...
 * of the metadata, when e.g. storing the print on device memory. It is
 * undefined whether this print is filled in by the driver and returned, or
 * whether the driver will return a newly created print after enrollment
 * succeeded.
 */
void fp_device_enroll(FpDevice *device, FpPrint *template_print,
  ...
  if (!FP_IS_PRINT(template_print)) {
    g_task_return_error(
        task, fpi_device_error_new_msg(FP_DEVICE_ERROR_DATA_INVALID,
  ...
  g_task_set_task_data(priv->current_task, data,
                       (GDestroyNotify)enroll_data_free);
...
static void match_data_free(FpMatchData *data) {
  g_clear_object(&data->print);
...
void fp_device_verify(FpDevice *device, FpPrint *enrolled_print,
  ...
  if (!cls->verify || !(priv->features & FP_DEVICE_FEATURE_VERIFY)) {
    g_task_return_error(
        task, fpi_device_error_new_msg(FP_DEVICE_ERROR_NOT_SUPPORTED,
 ...
 * @print: (out) (transfer full) (nullable): Location to store the scanned
 * print, or %NULL to ignore
 * @error: Return location for errors, or %NULL to ignore
 *
 * Finish an asynchronous operation to verify an enrolled print. You should
 * check for an error of type %FP_DEVICE_RETRY to prompt the user again if there
 * was an interaction issue.
 *
 * With @print you can fetch the newly created print and retrieve the image data
 * if available.
 *
 ...
 * @device: A #FpDevice
 * @result: A #GAsyncResult
 * @match: (out) (transfer full) (nullable): Location for the matched #FpPrint,
 * or %NULL
 * @print: (out) (transfer full) (nullable): Location for the new #FpPrint, or
 * %NULL
 * @error: Return location for errors, or %NULL to ignore
 *
...
void fp_device_delete_print(FpDevice *device, FpPrint *enrolled_print,
  ...
  g_task_set_task_data(priv->current_task, g_object_ref(enrolled_print),
                       g_object_unref);
 ...
 *
 * Returns: (element-type FpPrint) (transfer container): Array of prints or
 * %NULL on error
 */
...
static void async_result_ready(GObject *source_object, GAsyncResult *res,
                               gpointer user_data) {
...
FpPrint *fp_device_enroll_sync(FpDevice *device, FpPrint *template_print,
  ...
  fp_device_enroll(device, template_print, cancellable, progress_cb,
                   progress_data, NULL, async_result_ready, &task);
  while (!task)
    g_main_context_iteration(NULL, TRUE);
...
gboolean fp_device_verify_sync(FpDevice *device, FpPrint *enrolled_print,
  ...
  fp_device_verify(device, enrolled_print, cancellable, match_cb, match_data,
                   NULL, async_result_ready, &task);
  while (!task)
    g_main_context_iteration(NULL, TRUE);
...
gboolean fp_device_identify_sync(FpDevice *device, GPtrArray *prints,
  ...
  fp_device_identify(device, prints, cancellable, match_cb, match_data, NULL,
...
FpImage *fp_device_capture_sync(FpDevice *device, gboolean wait_for_finger,
  ...
  fp_device_capture(device, wait_for_finger, cancellable, async_result_ready,
                    &task);
  while (!task)
    g_main_context_iteration(NULL, TRUE);
...
GPtrArray *fp_device_list_prints_sync(FpDevice *device,
  ...
  fp_device_list_prints(device, NULL, async_result_ready, &task);
  while (!task)
    g_main_context_iteration(NULL, TRUE);
...
gboolean fp_device_clear_storage_sync(FpDevice *device,
  ...
  fp_device_clear_storage(device, cancellable, async_result_ready, &task);
  while (!task)
    g_main_context_iteration(NULL, TRUE);
